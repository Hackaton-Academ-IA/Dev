...
function PriceCard({ featured, kicker, title, price, period, perks, cta, ctaTone, accent, accentTbar }: PriceCardProps) {
  return (
...
function ModuleRow({ icon, name, desc, price, tag }: { icon: React.ReactNode; name: string; desc: string; price: string; tag?: string }) {
  return (
    <div className="grid grid-cols-[auto_1fr_auto] gap-3 sm:gap-5 items-center border-b-4 border-black last:border-b-0 p-4 hover:bg-[#0e0a22]">
      ...
      <div className="text-right">
        ...
        <button className="arcade arcade-ghost text-[8px] mt-2">+ AJOUTER</button>
      </div>
    </div>
  );
}
    ...
    <section id="tarifs" className="space-y-5">
      ...
      <div className="panel panel-gold mt-4">
        ...
        <div className="p-2 sm:p-3">
          <ModuleRow icon={<BrainIcon size={44} />}  name="MASTERY · MATHS DU LYCÉE"      desc="120+ quêtes adaptatives, 4 boss de chapitre, fiches de loot."            price="14,90€" tag="BUNDLE -30%" />
          <ModuleRow icon={<TrophyIcon size={44} />} name="CONCOURS · PRÉPA SCIENCES PO"  desc="Méthodologie + actu + 6 essais notés par l'IA."                          price="29,90€" tag="EARLY ACCESS" />
          <ModuleRow icon={<SwordIcon size={44} />}  name="LANGUES · ANGLAIS B2 → C1"     desc="Donjon de conversation, 30 boss de prononciation."                       price="19,90€" />
          <ModuleRow icon={<HeartIcon size={44} />}  name="SOFT SKILLS · LEADERSHIP"       desc="Scénarios narratifs ramifiés, debrief IA personnalisé."                  price="24,90€" tag="POUR LA GUILDE" />
        </div>
      </div>
